...
ylogdiff12 = [ylogdiff[t]-ylogdiff[t-12] for t in range(12,len(ylogdiff))]
# fit AR(2) model
...
# Invert diff(12)
ylogdiff_pred = ylogdiff[:12] + ylogdiff12_pred # concat ylogdiff12_pred
for i in range(12, len(ylogdiff_pred)): ylogdiff_pred[i] += ylogdiff_pred[i - 12]
# Invert diff(1)
ylog_pred = [ylog[0]] + ylogdiff_pred
for i in range(1, len(ylog_pred)): ylog_pred[i] += ylog_pred[i - 1]
